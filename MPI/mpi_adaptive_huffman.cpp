...
int main(int argc, char** argv) 
{
    ...
    // vector<string> fin = final;
   
    // if(my_rank == 0){
    //     total=end-start;
    //     cout<<"Encoded Length: "<<tot_size<<endl;
    ...
    // string decoded=Decoder(fin[my_rank],e,r);
    // // cout << encoded << endl;
    // if(my_rank == 0){
    //     dec[0] = decoded;
    //     // cout << nprocs << endl;
    ...
    // else{
    //     // cout << "SZ " << encoded.size() << endl;
    //     MPI_Send(decoded.c_str(), decoded.size(), MPI_CHAR, 0, 0, MPI_COMM_WORLD);
    //     // cout << "YES" << endl;
    // }
    // end=omp_get_wtime();
    // MPI_Finalize();
...
=>calculate it path (add it to encoded bits)
=>update the frequency for the character received
=>build the tree
=>swap if necessary
...
=>traverse to NYT node and add childs
left child -> new NYT node
right child -> the character received

=>build the tree
=>swap if necesary
=>old NYT code + fixed code and add it to encoded bits
...
/**
traverse the bits
    if reached node is NYT node
        read the first e bits and calculate the symbol add the new symbol in tree
    else
        read the symbol and reiterate
*/  